...
public class Utils : MonoBehaviour{
    ...
    /// <summary>
    /// While most Bezier curves are 3 or 3 points, it is possible to have 
    ...
    static public Vector3 Bezier( float u, params Vector3[] points ) {
        ...
        for ( int c = 0; c < points.Length; c++ ) {
            vArr[r, c] = points[c]; // book says vlist[c] but that throws an error, so.... I think points is what they meant
        }

        // Iterate over all remaining rows and interpolate points at each
        for ( r--; r >= 0; r-- ) {
            for ( int c = 0; c <= r; c++ ) {
